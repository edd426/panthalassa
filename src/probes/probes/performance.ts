...
// One simulated generation per wall-clock second at 1,000 organisms and 900
// ticks/generation requires 1,000 × 900 = 0.9M organism-ticks/s. Promotion to
// a gate requires repeated measurements on a documented reference environment.
export const THROUGHPUT_TARGET = 0.9e6;
...
export function evaluatePerformance(seed: string): ProbeReport {
  ...
  return makeReport({
    ...
    severity: 'warn',
    value: perSecond,
    threshold: { min: THROUGHPUT_TARGET, label: '≥0.9e6 organism-ticks/s' },
